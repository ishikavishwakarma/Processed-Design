...
document.addEventListener("DOMContentLoaded", function() {
	const sidebar = document.querySelector(".course_details-sidebar");
	const section = document.querySelector(".course_details-area");
	const footer = document.querySelector("footer");

	function updateSidebarPosition() {
		const sectionTop = section.offsetTop;
		const sectionHeight = section.offsetHeight;
		const sectionScrollPoint = sectionTop + (sectionHeight * 0.5); // 70% of the section height
		...
		if (screenWidth > 1024) {
			if (scrollTop >= sectionScrollPoint && scrollTop + sidebar.offsetHeight < footerTop) {
				sidebar.classList.add("fixed-sidebar");
			} else {
				sidebar.classList.remove("fixed-sidebar");
			}

			// Ensure the sidebar stops before the footer
			if (scrollTop + sidebar.offsetHeight >= footerTop - 90) {
				sidebar.style.position = 'absolute';
				sidebar.style.top = (footerTop - sidebar.offsetHeight - sectionTop) + 'px';
			} else {
				sidebar.style.position = 'fixed';
				sidebar.style.top = '90px'; // Fixed 40px from the top
...
document.getElementById('courses').addEventListener('click', function(e) {
	e.preventDefault();
	document.getElementById('course_div').scrollIntoView({ behavior: 'smooth' });
});
